          ...
          <SheetHeader>
            <SheetTitle className="flex items-center">
              <WaspRouterLink to={routes.LandingPageRoute.to}>
                <span className="sr-only">UnderstandYourPartner</span>
...
const NavLogo = ({ isScrolled }: { isScrolled: boolean }) => (
  <div className="relative">
    <img
      className={cn("transition-all duration-500 dark:hidden", {
        "size-8": !isScrolled,
        "size-7": isScrolled,
      })}
      src={logo}
      alt="UnderstandYourPartner App"
    />
    <img
      className={cn("hidden transition-all duration-500 dark:block", {
        "size-8": !isScrolled,
        "size-7": isScrolled,
      })}
      src={logoDark}
